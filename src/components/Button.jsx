...
const Button = ({ beer, ...props }) => {
  const classes = useStyles();
  const dispatch = useDispatch();
  const favourites = useSelector((store) => store.beers.favourite);

  const [active, setActive] = useState({ status: false });

  useEffect(() => {
    if (props.favourite.beersID.find((item) => item === beer.id)) {
